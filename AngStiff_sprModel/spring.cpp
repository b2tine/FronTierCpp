...
int main(int argc, char **argv)
{
	...
	level_func_pack.pos_component = FLUID_COMP2; //FLUID_COMP2 = 3
	...
	Front *front)
{
	...
	FT_VectorMemoryAlloc((POINTER*)&vparams,num_curves,sizeof(CURVE_VEL_PARAMS));
    ...
    for (i = 0; i < num_curves; ++i)
	{
	    ...
	    fscanf(infile,"%s",string);
	    (void) printf("%s\n",string);
	    if (string[0] != 'y' && string[0] != 'Y') continue;
	...
	double dt)
{
    ...
    if (hsbdry_type(oldc) != PRESET_CURVE || oldc->vel_pack == nullptr)
	{
	    if (debugging("trace"))
    ...
    if (vfunc_pack->func == nullptr || vfunc_pack->func_params == nullptr)
    {
        printf("ERROR: vfunc_pack->func_params == NULL\n");
        LOC(); clean_up(ERROR);
    }
    
    POINTER vparams = vfunc_pack->func_params; 
