...
extern void initSpringModel(Front *front)
{
    ...
    {
        char gvdir[100];
        sprintf(gvdir,"%s/module-step-1",OutName(front));
        printf("gvdir: %s\n",gvdir);
...
static void initSpringModel3d(Front *front)
{
	...
	if (CursorAfterStringOpt(infile,"Enter yes to use cgal:"))
    {
        ...
        {
            CursorAfterString(infile,"Enter cgal vtk file name:");
            fscanf(infile,"%s",cgal_vtk_name);
            (void) printf("%s\n",cgal_vtk_name);
            if (!read_vtk_surface(front->interf,amb_comp+1,amb_comp,cgal_vtk_name,&surf))
            {
        ...
        {
            FT_MakePlaneSurf(front,plane_nor,plane_pt,NO,
                amb_comp+1,amb_comp,ELASTIC_BOUNDARY,&surf);
        }
    }
	else
    {
