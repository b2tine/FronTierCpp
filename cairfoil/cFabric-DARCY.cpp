...
void CFABRIC_CARTESIAN::applicationSetComponent()
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (cell_center[i].comp == 2 || cell_center[i].comp == 4)
            cell_center[i].elastic = true;
        else
            cell_center[i].elastic = false;
    }
	...
	int nb)
{
	...
	int index0 = d_index(ic,top_gmax,dim);
    comp = cell_center[index0].comp;
            ...
            else
            {
                if (!status) /* extreme cases */
                {
                    ...
                    if(!status)
                    {
                        ...
                        if (!status)
                        {
                            ...
                            ///////////////////////////////////////////////
                            //ic_tmp[idir]++;
                            //int index_next = d_index(ic_tmp,top_gmax,dim);
                            for (k = i; k <= nrad; ++k)
                            {
                                vst->dens[nrad-k] = m_vst->dens[index0];
                                vst->engy[nrad-k] = m_vst->engy[index0];
                                vst->pres[nrad-k] = m_vst->pres[index0];
                                ...
                                if (dim == 1)
                                {
                                    vst->momn[0][nrad-k] = m_vst->momn[0][index0];
                                }
                                else if (dim == 2)
                                {
                                    for(j = 0; j < 2; j++)
                                    {
                                        vst->momn[j][nrad-k] = m_vst->momn[ind2[idir][j]][index0];
                                    }
                                }
                                else if (dim == 3)
                                {
                                    for (j = 0; j < 3; j++)
                                    {
                                        vst->momn[j][nrad-k] = m_vst->momn[ind3[idir][j]][index0];
                                    }
                                }
                            }
                            break;
                ...
                case FIRST_PHYSICS_WAVE_TYPE:
                    if (eqn_params->tracked)
                    {
                        ...
                        ghost_st.pres = m_vst->pres[index];
                ...
                if (!status) /* extreme cases */
                {
                    ...
                    if(!status)
                    {
                        ...
                        if (!status)
                        {
                            ...
                            for (k = i; k <= nrad; ++k)
                            {
                                vst->dens[n+nrad+k-1] = m_vst->dens[index0];
                                vst->engy[n+nrad+k-1] = m_vst->engy[index0];
                                vst->pres[n+nrad+k-1] = m_vst->pres[index0];
                                ...
                                if (dim == 1)
                                {
                                    vst->momn[0][n+nrad+k-1] = m_vst->momn[0][index0];
                                }
                                else if (dim == 2)
                                {
                                    for(j = 0; j < 2; j++)
                                    {
                                        vst->momn[j][n+nrad+k-1] = m_vst->momn[ind2[idir][j]][index0];
                                    }
                                }
                                else if (dim == 3)
                                {
                                    for (j = 0; j < 3; j++)
                                    {
                                        vst->momn[j][n+nrad+k-1] = m_vst->momn[ind3[idir][j]][index0];
                                    }
                                }
                            }
                            break;
	...
	COMPONENT	comp)
{
    ...
    /*
    FT_StateStructsAtGridCrossing(front,grid_intfc,icoords,
            dir[idir][nb],&sl,&sr,&hs,&hse,crx_coords);

    STATE* sleft = (STATE*)sl;
    STATE* sright = (STATE*)sr;

    double pl = sleft->pres;
    double rhol = sleft->dens;
    double pr = sright->pres;
    double rhor = sright->dens;

    double gamma = 1.4;

    double sgn = (rhol*pl - rhor*pr > 0) ? 1.0 : -1.0;
    double Msqr = gamma/(1.0 + gamma)*std::abs(rhol*pl - rhor*pr);
    ...
    case 2:
        {
            ...
            if (comp == negative_component(hs)) //if (gas_comp(negative_component(hs)))
            {
                ss = (STATE*)left_state(Bond_of_hse(hse)->start);
                se = (STATE*)left_state(Bond_of_hse(hse)->end);
            }
            else if (comp == positive_component(hs)) //else if (gas_comp(positive_component(hs)))
            {
