	...
	int dir)
{
	...
	case 1:
	    ...
	    else if (rect_boundary_type(intfc,dir,0) == NEUMANN_BOUNDARY || rect_boundary_type(intfc,dir,0)==ELASTIC_BOUNDARY)
	    {
		for (l = 1; l <= 3; ++l)
		{
		    j = imin[1] + l - 1;
	    ...
	    else if (rect_boundary_type(intfc,dir,1) == NEUMANN_BOUNDARY || rect_boundary_type(intfc,dir,1)==ELASTIC_BOUNDARY)
	    {
		for (l = 1; l <= 3; ++l)
		{
		    j = imax[1] - l + 1;
	...
	case 2:
	    i = i1;	j = i2;
	    if (rect_boundary_type(intfc,dir,0) == NEUMANN_BOUNDARY || rect_boundary_type(intfc,dir,0)==ELASTIC_BOUNDARY)
	    {
		for (l = 1; l <= 3; ++l)
		{
		    k = imin[2] + l - 1;
		...
		if (!FT_StateStructAtGridCrossing(front,grid_intfc,icoords,
			ldir[dir],comp,(POINTER*)&state,&hs,crx_coords))
		{
		    printf("In appendStencilBuffer3d()\n");
		    printf("ERROR: No crossing found!\n");
		...
		{
		case DIRICHLET_BOUNDARY:
//		    printf("Dirichlet boundary = %f %f %f\n",getStateXvel(state),getStateYvel(state),getStateZvel(state));
		    setDirichletStates(state,vst,m_vst,hs,icoords,dir,0,0,1);
		    break;
		default: 
		    printf("ERROR appendStencilBuffer2d: "
	    ...
	    if (rect_boundary_type(intfc,dir,1) == NEUMANN_BOUNDARY || rect_boundary_type(intfc,dir,1)==ELASTIC_BOUNDARY)
	    {
		for (l = 1; l <= 3; ++l)
		{
		    k = imax[2] - l + 1;
...
//		    printf("Dirichlet boundary = %f %f %f\n",getStateXvel(state),getStateYvel(state),getStateZvel(state));
		    offset=imax[dir]-imin[dir]+nrad;
		    setDirichletStates(state,vst,m_vst,hs,icoords,dir,1,offset,1);
		    break;
		default: 
		    printf("ERROR appendStencilBuffer2d: "
		    	   "unknown boundary type %d\n", wave_type(hs));
	...
	COMPONENT	comp)
{
	...
	double		*vel_ref = state->vel;//normal vel of interface
	double		coords[MAXD],coords_ref[MAXD],crx_coords[MAXD];
    ...
    //nor points back toward the comp we are in.
	FT_NormalAtGridCrossing(front,icoords,dir,comp,nor,&hs,crx_coords);

    //make nor point towards comp on other side of the interface
    for (i = 0; i < 3; ++i)
        nor[i] = -nor[i];
   
    //Interpolate fluid state at interface crossing, and use for
    //constant extrapolation across interface
    //
    //TODO: try using comp = NO_COMP
	...
	*/
	...
	for (i = istart; i <= nrad; ++i)
	{
	    ...
	    //if nb = 0, i.e. the point is on the upper of the boundary,
        //              we select three points below the boundary
	    //if nb = 1, i.e. the point is on the lower of the boundary,
        //              we select three points above the boundary
        ...
        */
	    ...
	    {
            //v[j] = st_tmp.momn[j]/st_tmp.dens - vel_ref[j];
            v[j] = st_tmp.momn[j]/st_tmp.dens;
            vn += v[j]*nor[j];
	    }
	    
        //v_tmp=v_ref-((v_tmp-v_ref) \cdot n)  n 
        for (j = 0; j < dim; j++)
            v[j] = vn*nor[j] - vel_ref[j];
            //v[j] = vel_ref[j] - 1.0*vn*nor[j];
