...
void CollisionSolver3d::resolveCollision()
{
    ...
    if (debugging("strain_limiting")) //if (!debugging("strainlim_off"))
    {
        limitStrainPosnJac();
            computeMaxSpeed(); //debug
        //TODO: this might be interfering with proximity impulses...
...
void CollisionSolver3d::aabbCollision()
{
    ...
    else
    {
        ...
        abt_collision->updateAABBTree(hseList);
            //abt_collision->collisionHseSet.clear();
...
void CollisionSolver3d::computeImpactZoneGS()
{
    ...
    while(is_collision)
    {
        ...
        if (debugging("collision"))
        {
            ...
            //TODO: report number impzone orphans
...
void CollisionSolver3d::computeImpactZoneJac()
{
    ...
    while(is_collision)
    {
        ...
        if (is_collision)
        {
            updateAverageVelocity(MotionState::MOVING);
...
void CollisionSolver3d::debugImpactZones()
{
    ...
    //find orphan collisions
    std::vector<POINT*> impzone_orphans;
    unsortHseList(hseList);
	...
	for (auto it = hseList.begin(); it < hseList.end(); ++it)
    {
	    for (int i = 0; i < (*it)->num_pts(); ++i)
        {
            ...
            STATE* sl = (STATE*)left_state(pt);
            if (sl->impzone_orphan)
            {
                impzone_orphans.push_back(pt);
            }
    ...
    if (!impzone_orphans.empty())
    {
        std::string fname = outdir + "/impzone_orphans";
        vtk_write_pointset(impzone_orphans,fname,0);
    }
...
void CollisionSolver3d::infoImpactZones()
{
    ...
    //TODO: Only report elastic hse impact zones
	unsortHseList(elasticHseList);
	//unsortHseList(hseList);
        ...
        MotionState mstate)
{
	...
	for (auto it = list.begin(); it < list.end(); ++it)
    {
        ...
        for (int i = 0; i < ne; ++i)
        {
            ...
            /*
            //skip edges that belong to an impact zone
            if (is_detImpZone)
            {
                if (weight(p[0]) > 1 && weight(p[1]) > 1 &&
                        findSet(p[0]) == findSet(p[1])) continue;
            }
            */
...
int CollisionSolver3d::computeStrainImpulsesVel(std::vector<CD_HSE*>& list)
{
	...
	for (auto it = list.begin(); it < list.end(); ++it)
    {
        ...
        for (int i = 0; i < ne; ++i)
        {
            ...
            //in limitStrainPos() or limitStrainRatePos() or limitStrainVel() (this function)
