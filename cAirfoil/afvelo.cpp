...
void setMotionParams(Front* front)
{
	...
	if (af_params->no_fluid == NO)
	{
        ...
        */
        ...
        {
		        // default: no porosity
                cFparams->with_porosity = af_params->with_porosity = NO;
                if(CursorAfterStringOpt(infile,"Enter yes to use porosity:"))
		        {
                    fscanf(infile,"%s",string);
                    (void) printf("%s\n",string);
                    if (string[0] == 'y' || string[0] == 'Y')
                	cFparams->with_porosity = af_params->with_porosity = YES;
		        }

                /*
                if (af_params->with_porosity == YES)
                {
	...
	for (i = 0; i < dim; ++i)
	    af_params->gravity[i] = cFparams->gravity[i];
