...
#include "crystal.h"
#define		UNSTABLE_CELL_FRAC	0.15
...
void C_CARTESIAN::setComponent(void)
{
	...
	if (is_conservative(cRparams->num_scheme))
	{
		...
		for (l = 0; l < cell_part[i].ns_new;++l)
		    cell_part[i].icn_old_send[l] = 
				cell_part[i].icn_new_send[l];
		cell_part[i].unstable_cell = NO;
		cell_part[i].solute_volume = 0.0;
	    }
...
void C_CARTESIAN::computeAdvection(void)
{
	...
	if (is_conservative(cRparams->num_scheme))
	{
	    ...
	    const GRID_DIRECTION dir[3][3] = {{WEST,CENTER,EAST},
	    				      {SOUTH,CENTER,NORTH},
					      {LOWER,CENTER,UPPER}};
	    GRID_DIRECTION dirs[3];
	    boolean flag;
	    parab_solver.cell_part = field->cell_part;
	    
	    if (dim == 3)
	    {
	   	double full_cell_vol = top_h[0]*top_h[1]*top_h[2];
	        double stable_solute_vol = UNSTABLE_CELL_FRAC*full_cell_vol;
		CELL_PART *cell_part = field->cell_part;
		/*
		...
		{	
		    ...
		    if (top_comp[ic] == SOLUTE_COMP && 
		       cell_part[ic].vol_new[1] < stable_solute_vol)
		    {
		      cell_part[ic].unstable_cell = YES;
		      for (ii = 0; ii < 3; ++ii)
		      {
			for (jj = 0; jj < 3; ++jj)
			{
			  for (kk = 0; kk < 3; ++kk)
			  {
			    ...
			    if (FT_AdjTopGridIcoords(front,icoords,dirs,ipn) &&
			        top_comp[icn] == SOLUTE_COMP &&
				cell_part[icn].vol_new[1] >= stable_solute_vol)
			    {
			        flag = YES;
			        cell_part[icn].vol_new[1] += 
					cell_part[ic].vol_new[1];
				...
				cell_part[ic].ns_new++;
				cell_part[ic].vol_new[1] = 0.0;
				top_comp[ic] = CRYSTAL_COMP;
				break;
			    }
			  }	
			  if (flag == YES)
			...
			if (flag == YES)
			    break;
                      ...
                      if (flag == NO)
		      {
			    printf("Cannot find a neighbor to transfer ");
			    printf("SOLUTE_COMP in unstable cell(%d,%d,%d)\n"
			    		,i,j,k);
			    clean_up(ERROR);
		      }
	...
	*/	
	...
	/*  comment before solveCEX() is fully working */
            if (dim < 3 && m_dt < 0.1*sqr(hmin)/cRparams->D/(double)dim)
	    	parab_solver.solveCEX();
	    else
	...
	if (debugging("neg_solute"))
	{
		...
		for (k = kmin; k <= kmax; ++k)
		{
		    ...
		    if (field->solute[ic] < -0.001)
		    {
			printf("At i = %d j = %d k = %d: solute = %20.14f\n",
                                i,j,k,field->solute[ic]);
			printf("m_dt = %f\n",m_dt);
			clean_up(ERROR);
		    }	    
		}
...
void C_CARTESIAN::setAdvectionDt()
{
	...
	{
	    printf("Time step from c_cartesian.max_dt = %24.10g\n",max_dt);
	}
}	/* end setAdvectionDt */
...
void C_CARTESIAN::readFrontInteriorStates(char *restart_name)
{
	...
	next_output_line_containing_string(infile,"Conservative variables:");
	fclose(infile);
	setAdvectionDt();
...
void C_CARTESIAN::initSampleSolute(char *in_name)
{
        ...
        {
            CursorAfterString(infile,"Enter the sample line type:");
            fscanf(infile,"%s %s",sample_type, sample_type+1);
	...
	boolean use_old_vol)
{
	...
	*total_solute = *total_crystal = 0.0;
	*vol_solute = *vol_crystal = 0.0;
	switch(dim)
	{
	case 1:
	    intfc_1d_data(front,&posn,&solt);
		...
		else if (top_comp[ic] == CRYSTAL_COMP)
		{
		    ...
		    *total_crystal += field->solute[ic]*cell_vol;
		    *vol_crystal += cell_vol;
		}
	    }
	    break;
	case 2:
