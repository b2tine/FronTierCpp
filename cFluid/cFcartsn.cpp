...
void G_CARTESIAN::setDomain()
{
	...
	if (first)
	{
        ...
        FT_VectorMemoryAlloc((POINTER*)&eqn_params->mu_turb,size,sizeof(double));
	    ...
	    field.mu_turb = eqn_params->mu_turb;
	...
	SWEEP *vst)
{
	...
	FT_VectorMemoryAlloc((POINTER*)&vst->mu_turb,size,sizeof(double));
...
void G_CARTESIAN::freeDirVstFlux(
        ...
        FSWEEP* flux)
{
    FT_FreeThese(8,vst->dens,vst->engy,vst->pres,vst->momn,
            vst->mu,vst->mu_turb,vst->temp,vst->k_turb);
...
void G_CARTESIAN::printFrontInteriorStates(char *out_name)
{
    ...
    while (next_point(intfc,&p,&hse,&hs))
    {
        ...
        fprintf(outfile,"%*.*f %*.*f\n",WID,DEC,getStateTemp(sl),
            WID,DEC,getStateTemp(sr));
        
        fprintf(outfile,"%*.*f %*.*f\n",WID,DEC,getStateKTurb(sl),
            WID,DEC,getStateKTurb(sr));
        
        fprintf(outfile,"%*.*f %*.*f\n",WID,DEC,getStateMu(sl),
            WID,DEC,getStateMu(sr));
        
        fprintf(outfile,"%*.*f %*.*f\n",WID,DEC,getStateMuTurb(sl),
            WID,DEC,getStateMuTurb(sr));
	...
	case 1:
	    for (i = 0; i <= top_gmax[0]; ++i)
	    {
		    index = d_index1d(i,top_gmax);
            fprintf(outfile,"%*.*f\n",WID,DEC,temp[index]);
            fprintf(outfile,"%*.*f\n",WID,DEC,k_turb[index]);
            fprintf(outfile,"%*.*f\n",WID,DEC,mu[index]);
            fprintf(outfile,"%*.*f\n",WID,DEC,mu_turb[index]);
	        fprintf(outfile,"%*.*f\n",WID,DEC,dens[index]);
	        fprintf(outfile,"%*.*f\n",WID,DEC,engy[index]);
	    	for (l = 0; l < dim; ++l)
            {
	...
	char		*restart_name)
{
    ...
    while (next_point(intfc,&p,&hse,&hs))
    {
        ...
        fscanf(infile,"%lf %lf",&lstate->temp,&rstate->temp);
        fscanf(infile,"%lf %lf",&lstate->k_turb,&rstate->k_turb);
        fscanf(infile,"%lf %lf",&lstate->mu,&rstate->mu);
        fscanf(infile,"%lf %lf",&lstate->mu_turb,&rstate->mu_turb);
...
void G_CARTESIAN::readInteriorStates(char *restart_name)
{
	...
	case 2:
	    ...
	    for (j = 0; j <= top_gmax[1]; ++j)
	    {
            ...
            fscanf(infile,"%lf",&k_turb[index]);
            fscanf(infile,"%lf",&mu[index]);
            fscanf(infile,"%lf",&mu_turb[index]);
            fscanf(infile,"%lf",&dens[index]);
            fscanf(infile,"%lf",&engy[index]);
            
            st_tmp.mu = mu[index];
            st_tmp.mu_turb = mu_turb[index];
            st_tmp.dens = dens[index];
            st_tmp.engy = engy[index];
            st_tmp.k_turb = k_turb[index];
            st_tmp.temp = temp[index];
...
void G_CARTESIAN::initMovieVariables()
{
	...
	case 2:
        ...
        CursorAfterString(infile,"Type y to make movie of turb kinetic energy:");
        fscanf(infile,"%s",string);
        (void) printf("%s\n",string);
        if (string[0] == 'Y' || string[0] == 'y')
        {
            if (set_bound)
            {
                CursorAfterString(infile,"Enter min and max turb kinetic energy:");
                        fscanf(infile,"%lf %lf",&var_min,&var_max);
                        (void) printf("%f %f\n",var_min,var_max);
            }
            
            FT_AddHdfMovieVariable(front,set_bound,YES,SOLID_COMP,
                    "k_turb",0,eqn_params->k_turb,getStateKTurb,
                    var_max,var_min);
        }
        ...
        if (string[0] == 'Y' || string[0] == 'y')
	    {
            ...
            FT_AddHdfMovieVariable(front,set_bound,YES,SOLID_COMP,
                    "temp",0,eqn_params->temp,getStateTemp,var_max,var_min);
	    }
	...
	if (dim != 1)
	{
	    ...
	    if (CursorAfterStringOpt(infile,
               "Type y to make scalar temperature field movie:"))
            {
                ...
                (void)printf("%s\n",string);
                if (string[0] == 'Y' || string[0] == 'y')
                    FT_AddVtkScalarMovieVariable(front,"TEMP",field.temp);
            }
	    if (CursorAfterStringOpt(infile,
               "Type y to make scalar turb kinetic energy field movie:"))
            {
                fscanf(infile,"%s",string);
                (void)printf("%s\n",string);
                if (string[0] == 'Y' || string[0] == 'y')
                    FT_AddVtkScalarMovieVariable(front,"KTURB",field.k_turb);
            }
	}
...
void G_CARTESIAN::copyMeshStates()
{
	...
	case 1:
	    ...
	    FT_ParallelExchGridArrayBuffer(mu_turb,front,NULL);
...
{
	FT_ParallelExchGridArrayBuffer(m_vst->dens,front,nullptr);
	...
	FT_ParallelExchGridArrayBuffer(m_vst->mu_turb,front,nullptr);
	...
	SWEEP *m_vst)
{
	...
	case 1:
	    for (i = 0; i <= top_gmax[0]; ++i)
	    {
            ...
            m_vst->mu_turb[index] = m_vst_orig.mu_turb[index];
	...
	SWEEP *m_vst)
{
	...
	case 1:
	    for (i = 0; i <= top_gmax[0]; ++i)
	    {
            ...
            m_vst->mu_turb[index] = mu_turb[index];
...
//NOTE: DO NOT COPY mu_turb back to field.mu_turb !!!!!!!
//
...
void G_CARTESIAN::freeVst(
	SWEEP *vst)
{
	FT_FreeThese(8,vst->dens,vst->momn,vst->engy,vst->pres,
            vst->mu,vst->mu_turb,vst->k_turb,vst->temp);
}	/* end freeVst */
	...
	case 0:
		...
		else
		{
            ...
            case SYMMETRY_BOUNDARY:
		    	setSymmetryStates(vst,m_vst,hs,state,ic_next,idir,
						nb,0,i,comp);
		    	break;
		    case DIRICHLET_BOUNDARY:
...
void G_CARTESIAN::setSymmetryStates(
	SWEEP		*vst,
	SWEEP		*m_vst,
	HYPER_SURF 	*hs,
	STATE		*state,
	...
	COMPONENT	comp)
{
	...
	if (debugging("symmetry_buffer"))
	{
	    (void) printf("Entering setSymmetryStates()\n");
	    (void) printf("comp = %d\n",comp);
	    (void) printf("icoords = %d %d %d\n",icoords[0],icoords[1],
				icoords[2]);
	    (void) printf("idir = %d nb = %d\n",idir,nb);
	...
	for (i = istart; i <= nrad; ++i)
	{
        ...
        // reflect normal component of velocity -- preserve relative tangential vel//
        for (j = 0; j < dim; ++j)
        {
            v[j] = vel_reflect[j] - 2.0*vn*nor[j];
    		st_tmp.momn[j] = v[j]*st_tmp.dens;
	...
	if (debugging("symmetry_buffer"))
	    (void) printf("Leaving setSymmetryStates()\n");
}	/* end setSymmetryStates */
    ...
    FSWEEP *m_flux)
{
	...
	while (seg_min <= imax[idir])
	{
        ...
        vst.k_turb[n+nrad] = m_vst->k_turb[index];
